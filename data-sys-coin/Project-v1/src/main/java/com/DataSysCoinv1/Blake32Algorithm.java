...
public class Blake32Algorithm {
    ...
    private Blake32Algorithm(byte[] key){
        initialize(wordsFromLEBytes(key), KEYED_HASH);
    }


    private Blake32Algorithm(String context){
        Blake32Algorithm contextHasher = new Blake32Algorithm();
     ...
     */
    public static Blake32Algorithm newInstance(){
        return new Blake32Algorithm();
    }
     ...
     */
    public static Blake32Algorithm newKeyedHasher(byte[] key){
        if(!(key.length == KEY_LEN)) throw new IllegalStateException("Invalid key length");
        return new Blake32Algorithm(key);
    }
     ...
     */
    public static Blake32Algorithm newKeyDerivationHasher(String context){
        return new Blake32Algorithm(context);
    }
}
